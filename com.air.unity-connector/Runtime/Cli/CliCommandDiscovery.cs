            ...
            lock (Gate)
            {
                ...
                foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
                {
                    ...
                    foreach (var type in types)
                    {
                        ...
                        {
                            LogWarning(
                                $"[unity-connector] CLI command type '{type.FullName}' cannot be instantiated: {ex.Message}");
                            continue;
                        }
                        ...
                        {
                            LogWarning(
                                $"[unity-connector] CLI command type '{type.FullName}' must inherit CliCommand or CliCommand<T>.");
                            continue;
                        }
                        ...
                        {
                            LogWarning(
                                $"[unity-connector] CLI command '{descriptor.Name}' does not implement ICliCommand / ICliCommand<T>.");
                            continue;
                        }
