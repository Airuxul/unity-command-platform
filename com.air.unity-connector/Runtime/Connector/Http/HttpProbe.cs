...
namespace Air.UnityConnector.Http
{
    public static class HttpProbe
    {
            ...
            string expectedSessionId = null)
        {
            ...
            var data = JsonSerialization.ParseObject(body);
            if (!ReadBool(data, "ok"))
                return false;
            ...
            string expectedSessionId = null)
        {
            ...
            if (expectedBuild.HasValue)
            {
                ...
                {
                    var got = data.TryGetValue("connector_build", out var raw) && TryReadInt(raw, out var parsed)
                        ? parsed.ToString()
                        : "null";
                    return $"connector_build mismatch (got {got}, expected {expectedBuild.Value})";
                }
            }
